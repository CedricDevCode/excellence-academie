    ...
    <div className="min-h-screen bg-gray-50">
      ...
      <div className="max-w-5xl mx-auto px-4 py-8">
        {submissions.length === 0 ? (
          <div className="bg-white rounded p-12 text-center shadow-sm border border-gray-100">
          ...
          <div className="space-y-6">
            {submissions.map((s: any) => (
              <div key={s.id} className="bg-white rounded shadow-sm border border-gray-100 p-6">
                ...
                {s.fileUrl && (
                  <a href={s.fileUrl} target="_blank" rel="noopener noreferrer"
                    className="inline-flex items-center gap-1.5 text-xs text-[#c97e00] font-semibold hover:underline mb-4">
                    <Download size={14} /> Télécharger le fichier joint
                  </a>
                )}

                {s.grade != null && (
                  <div className="mb-4 inline-flex items-center gap-2 px-3 py-1.5 bg-green-50 rounded">
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-3 gap-3">
                    ...
                    <div className="flex items-end">
                      <button onClick={() => handleEvaluate(s.id)} disabled={evaluating[s.id]}
                        className="flex items-center gap-1.5 px-4 py-2 bg-primary-600 text-white rounded text-sm font-semibold hover:bg-primary-700 disabled:opacity-50">
