        ...
        <div ref={ref}>
          ...
          >
            <div className="overflow-hidden rounded-2xl shadow-2xl border border-gray-200/60">
              <div className="relative w-full" style={{ aspectRatio: '21/9', minHeight: '340px', maxHeight: '620px' }}>
                      ...
                      >
                        ...
                        {(slide.title || slide.subtitle) && (
                          <div className="absolute inset-x-0 bottom-0 bg-gradient-to-t from-black/90 via-black/50 to-transparent p-8 sm:p-10 md:p-12 text-white">
                            {slide.title && <h3 className="font-extrabold text-xl sm:text-2xl md:text-3xl leading-tight drop-shadow-lg">{slide.title}</h3>}
                ...
                {slides.length > 1 && (
                  <>
                    <button onClick={() => setCurrentIndex((prev) => (prev === 0 ? slides.length - 1 : prev - 1))}
                      className="absolute left-4 top-1/2 -translate-y-1/2 w-12 h-12 rounded-full bg-white/90 hover:bg-white text-gray-800 flex items-center justify-center shadow-xl transition-all hover:scale-110 z-10">
                      <ChevronLeft size={24} />
                    </button>
                    <button onClick={() => setCurrentIndex((prev) => (prev + 1) % slides.length)}
                      className="absolute right-4 top-1/2 -translate-y-1/2 w-12 h-12 rounded-full bg-white/90 hover:bg-white text-gray-800 flex items-center justify-center shadow-xl transition-all hover:scale-110 z-10">
