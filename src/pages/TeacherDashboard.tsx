...
function SessionsView({ user }: { user: any }) {
  ...
  useEffect(() => { if (user?.id) loadSessions(); }, [user?.id, loadSessions]);
...
function PlanningView({ user }: { user: any }) {
  const [events, setEvents] = useState<any[]>([]);
  const [eventsLoading, setEventsLoading] = useState(true);

  useEffect(() => {
    if (user?.id) {
      fetchEvents({ teacherId: user.id })
        .then(setEvents)
        .catch(() => {})
                  ...
                  >
                    <Card className="p-5 hover:shadow-md transition-shadow">
                      ...
                      <div className="flex items-center gap-2 text-xs text-gray-500">
                        <Badge variant="primary">{s.ville || "Non défini"}</Badge>
                        <Badge variant={s.role === "STUDENT" ? "success" : "gray"}>
              ...
              <div className="space-y-4">
                {notifs.map((n: any, i: number) => (
                  <motion.div
                    ...
                    transition={{ delay: i * 0.05 }}
                  >
                    <Card className={`p-5 ${!n.isRead ? "border-l-4 border-l-primary-500" : ""}`}>
                      ...
                      {!n.isRead && (
                        <button onClick={async () => { await markNotificationRead(n.id); loadData(); }}
                          className="mt-2 text-primary-500 text-xs font-semibold hover:underline">
                          Marquer comme lu
                        </button>
                      )}
                    </Card>
            ...
            <div className="grid grid-cols-1 xl:grid-cols-2 gap-6">
              <Card>
                <div className="p-6 border-b border-gray-100">
                  <h2 className="font-black text-gray-900 flex items-center gap-2"><GraduationCap size={18} className="text-primary-500" /> Derniers étudiants inscrits</h2>
                </div>
              ...
              <Card>
                <div className="p-6 border-b border-gray-100">
                  <h2 className="font-black text-gray-900 flex items-center gap-2"><Bell size={18} className="text-primary-500" /> Dernières notifications</h2>
                </div>
                <div className="divide-y divide-gray-50">
                  {notifs.length === 0 ? (
                    ...
                    notifs.slice(0, 5).map((n: any) => (
                      <div key={n.id} className={`p-4 hover:bg-gray-50 transition-colors ${!n.isRead ? "bg-primary-50/30" : ""}`}>
        ...
        <div className="flex items-center justify-between p-5 border-b border-primary-500/30">
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <div className="text-primary-200 text-xs">Enseignant</div>
            </div>
          </div>
          <button onClick={() => setSidebarOpen(false)} className="lg:hidden text-primary-200 hover:text-white" aria-label="Fermer le menu">
            <X size={20} />
          </button>
        </div>
      ...
      <main className="flex-1 flex flex-col min-w-0 overflow-hidden">
        <header className="bg-white border-b border-gray-200 px-6 py-4 flex items-center justify-between shrink-0">
          ...
          <div className="flex items-center gap-3">
            ...
            <div className="w-9 h-9 rounded-full bg-primary-500 flex items-center justify-center text-white font-black text-xs" aria-hidden="true">{initials}</div>
          </div>
        </header>
