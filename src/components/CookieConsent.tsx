      ...
      <div className="max-w-2xl mx-auto bg-white rounded-2xl shadow-2xl border border-gray-200 p-5 sm:p-6">
        <div className="flex items-start gap-4">
          <div className="w-10 h-10 rounded-xl bg-blue-100 flex items-center justify-center shrink-0">
            <Shield size={20} className="text-[#0056B3]" />
          </div>
          <div className="flex-1 min-w-0">
            ...
            <div className="flex flex-wrap gap-2 mt-4">
              <button
                ...
                className="px-4 py-2 bg-[#0056B3] text-white text-xs font-bold rounded-lg hover:bg-[#003d80] transition-colors"
              >
                Tout accepter
              </button>
              <button
              ...
              <button
                onClick={onSettings}
                className="px-4 py-2 text-[#0056B3] text-xs font-bold rounded-lg hover:bg-blue-50 transition-colors flex items-center gap-1.5"
              >
                <Settings size={14} />
                Personnaliser
              </button>
    ...
    <div className="fixed inset-0 z-50 flex items-end sm:items-center justify-center p-4" role="dialog" aria-label="Paramètres des cookies">
      ...
      <div className="relative bg-white rounded-2xl shadow-2xl w-full max-w-lg max-h-[80vh] overflow-y-auto">
        ...
        <div className="px-6 py-4 space-y-4">
          {categories.map((cat) => (
            <div key={cat.id} className="flex items-start gap-4 p-4 bg-gray-50 rounded-xl border border-gray-100">
              <cat.icon size={20} className="text-[#0056B3] shrink-0 mt-0.5" />
              <div className="flex-1 min-w-0">
        ...
        <div className="sticky bottom-0 bg-white border-t border-gray-100 px-6 py-4 flex gap-3 rounded-b-2xl">
          <button
            ...
            className="flex-1 px-4 py-2.5 bg-[#0056B3] text-white text-sm font-bold rounded-lg hover:bg-[#003d80] transition-colors"
          >
            Enregistrer mes choix
          </button>
          <button
            onClick={onClose}
            className="px-4 py-2.5 bg-gray-100 text-gray-700 text-sm font-bold rounded-lg hover:bg-gray-200 transition-colors"
          >
            Annuler
          </button>
        </div>
